...
### Core `Game` Field Specification

* `id`: Unique identifier generated by Chess Vault upon persistence (MongoDB ObjectId string).
* `userId`: Clerk User ID of the account owning this game record.
* `folderIds`: Array of MongoDB folder ObjectIds this game belongs to, or `null` if unorganized.
* `platform`: Source platform (`"chess.com"` or `"lichess"`).
* `platformGameId`: Platform-assigned unique ID (e.g. Chess.com archive UUID or Lichess game ID).
* `sourceUrl`: Web URL to view the game on the source platform.
* `whitePlayer`: Object containing White player `username` and non-negative `rating`.
